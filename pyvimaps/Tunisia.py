...
class Tunisia():
    """
        Class Tunisia to handle the tunisian shapes file, format and plot the
        data as a geo stats plot.
    """

    def __init__(self):
        self.tunisia      = Country('TN')
        self.fig, self.ax = plt.subplots()
        self.tn_map       = Basemap(llcrnrlon  = self.tunisia.lower_longitude_bound-0.25,
                                    llcrnrlat  = self.tunisia.lower_lattitude_bound-1,
                                    urcrnrlon  = self.tunisia.upper_longitude_bound+3,
                                    urcrnrlat  = self.tunisia.upper_lattitude_bound+0.25,
                                    resolution = 'i',
                                    projection = 'merc',
                                    lat_0      = 34,
                                    lon_0      = 10)

        self.sahpe_info = self.tn_map.readshapefile('pyvimaps/shapefiles/TN/TN', 'states')
    ...
    def plot_data(self, data, title, cmap_name):
        ...
        colorbar_axis = self.fig.add_axes([0.6, 0.25, 0.01, 0.5]) # posititon
